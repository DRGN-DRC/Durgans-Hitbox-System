	...
	var methods={
		init:function(options){
			...
			function is_touch_device(){
				...
				return 0 // Until bug is fixed with mCSB_dragger scrollDone callback
			}
		...
		scroll:function(updated){
			...
			if(window.outerWidth && window.innerWidth){ /*fix webkit zoom and jquery animate*/
		...
		scrollTo:function(scrollTo,options,animationDuration){
			var defaults={
				moveDragger:false,
				callback:true
			},
			...
			if(scrollTo){
				...
				if($(document).data("mCS-is-touch-device")){ /*is touch device*/
					...
					//	mCSB_dragger.css("top",scrollToPos);
                        mCSB_dragger.stop().animate({top: scrollToPos}, animationDuration, "easeOutCirc", function() {onDataScrollDone($this);});
					}
                    /*
					if(options.callback){
						$this.mCustomScrollbar("scroll");
